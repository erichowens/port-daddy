...
const MAX_DNS_RECORDS_PER_IDENTITY = 50;
...
export function createDns(db: Database.Database) {
  ...
  const stmts = {
    ...
    countByIdentityPrefix: db.prepare("SELECT COUNT(*) as count FROM dns_records WHERE identity LIKE ? ESCAPE '\\'"),
   ...
   * Enforces a maximum of MAX_DNS_RECORDS_PER_IDENTITY (50) records sharing
   * the same identity prefix (first segment before ':').
   */
  function register(identity: string, options: RegisterOptions) {
    ...
    // Enforce max records per identity prefix (first segment before ':')
    const identityPrefix = trimmedIdentity.split(':')[0];
    if (identityPrefix) {
      const prefixPattern = identityPrefix.replace(/[%_]/g, '\\$&') + ':%';
      const prefixCount = (stmts.countByIdentityPrefix.get(prefixPattern) as { count: number }).count;
      if (prefixCount >= MAX_DNS_RECORDS_PER_IDENTITY) {
        return {
          success: false,
          error: `identity prefix "${identityPrefix}" has reached the maximum of ${MAX_DNS_RECORDS_PER_IDENTITY} DNS records`,
