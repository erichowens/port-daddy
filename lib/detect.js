...
export function suggestIdentity(dir = process.cwd()) {
  ...
  if (stack) {
    ...
    if (['Next.js', 'Nuxt', 'SvelteKit', 'Remix', 'Astro', 'Vite', 'Create React App', 'Angular', 'Vue CLI'].includes(stack.name)) {
      stackName = 'frontend';
    } else if (['Express', 'Fastify', 'Hono', 'NestJS', 'FastAPI', 'Flask', 'Django', 'Go', 'Rust'].includes(stack.name)) {
      stackName = 'api';
    } else if (['Cloudflare Workers'].includes(stack.name)) {
      stackName = 'worker';
    } else if (['Docker'].includes(stack.name)) {
      stackName = 'container';
