  ...
  return (
    <Box flexDirection="column">
      <Box gap={2} marginBottom={1}>
        <Box width={36}><Text {...COL}>IDENTITY</Text></Box>
      ...
      {services.slice(0, 30).map(s => (
        <Box key={s.id} gap={2}>
          <Box width={36}><TokenId id={s.id} /></Box>
          <Box width={6}><Text color="green" bold>{pad(s.port ?? '--', 6)}</Text></Box>
          <Box width={10}><Text color={s.status === 'active' ? 'green' : 'gray'}>{pad(s.status ?? 'active', 10)}</Text></Box>
          <Text dimColor>{timeAgo(s.lastSeen ?? s.last_seen)}</Text>
        </Box>
      ))}
      {services.length > 30 && <Text dimColor>{'… and ' + (services.length - 30) + ' more'}</Text>}
    </Box>
  );
}
...
function AgentsPanel({ agents }: { agents: Agent[] }): React.ReactElement {
  ...
  return (
    <Box flexDirection="column">
      ...
      {agents.map(a => (
        <Box key={a.id} gap={2}>
          <Box width={28}><TokenId id={a.id} /></Box>
          <Box width={28}><TokenId id={a.identity ?? '--'} /></Box>
          <Box width={12}><Text dimColor>{timeAgo(a.lastHeartbeat)}</Text></Box>
          <Text dimColor>{(a.purpose ?? '--').slice(0, 45)}</Text>
        </Box>
      ))}
    </Box>
  );
...
function SessionsPanel({ sessions }: { sessions: Session[] }): React.ReactElement {
  ...
  return (
    <Box flexDirection="column">
      <Box gap={2} marginBottom={1}>
        ...
        <Box width={12}><Text {...COL}>STATUS</Text></Box>
      ...
      {sorted.slice(0, 25).map(s => (
        <Box key={s.id} gap={2}>
          ...
          <Box width={25}><TokenId id={s.agentId ?? '--'} /></Box>
          <Box width={12}>
            <Text color={s.status === 'active' ? 'green' : s.status === 'abandoned' ? 'red' : 'gray'}>
              {pad(s.status, 12)}
            </Text>
          </Box>
...
function LocksPanel({ locks }: { locks: Lock[] }): React.ReactElement {
  if (!locks.length) return <Text dimColor>{'No active locks. Run: pd lock <name>'}</Text>;
  return (
    <Box flexDirection="column">
      <Box gap={2} marginBottom={1}>
...
function ChannelsPanel({ channels }: { channels: Channel[] }): React.ReactElement {
  if (!channels.length) return <Text dimColor>{'No active channels. Run: pd pub <channel> <msg>'}</Text>;
  return (
    <Box flexDirection="column">
      <Box gap={2} marginBottom={1}>
  ...
  return (
    <Box flexDirection="column" paddingX={1}>
      ...
      <Box borderStyle="single" borderColor="gray" paddingX={1} flexDirection="column" minHeight={18}>
        ...
        {tab === 'Sessions'  && <SessionsPanel  sessions={sess}  />}
        {tab === 'Locks'     && <LocksPanel     locks={lcks}     />}
        {tab === 'Channels'  && <ChannelsPanel  channels={chans} />}
      </Box>
