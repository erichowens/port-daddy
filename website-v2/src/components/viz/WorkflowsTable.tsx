...
export function WorkflowsTable() {
  ...
  const handleRun = async (rule: any) => {
    try {
      // Simulate trigger by publishing to the rule's channel
  ...
  return (
    <motion.div className="flex flex-col h-full bg-[var(--bg-surface)] rounded-3xl border border-[var(--border-default)] overflow-hidden shadow-sm font-sans">
      <motion.div className="px-8 py-6 border-b border-[var(--border-subtle)] flex items-center justify-between bg-[var(--bg-overlay)]">
        <motion.div>
        ...
        <motion.button 
          className="px-4 py-2 rounded-xl bg-[var(--brand-primary)] text-[var(--bg-base)] text-sm font-bold flex items-center gap-2 transition-all shadow-lg shadow-[var(--brand-primary)]/20"
        ...
        <motion.table className="w-full text-left border-collapse font-sans">
          <motion.thead>
            <motion.tr className="text-[10px] font-black uppercase tracking-[0.2em] text-[var(--text-muted)] border-b border-[var(--border-subtle)] font-sans">
              <motion.th className="px-8 py-4">Status</motion.th>
              <motion.th className="px-4 py-4">Rule Name</motion.th>
              <motion.th className="px-4 py-4">Trigger (Channel)</motion.th>
              <motion.th className="px-4 py-4">Action</motion.th>
              <motion.th className="px-8 py-4 text-right">Control</motion.th>
            </motion.tr>
          </motion.thead>
          <motion.tbody className="divide-y divide-[var(--border-subtle)]">
            {(rules || []).map((rule) => (
              <motion.tr 
                key={rule.id}
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                className="group hover:bg-[var(--bg-overlay)] transition-colors"
              >
                <motion.td className="px-8 py-5">
                ...
                <motion.td className="px-4 py-5 font-sans">
                  <motion.span className="font-bold text-sm" style={{ color: 'var(--text-primary)' }}>{rule.name}</motion.span>
                </motion.td>
                <motion.td className="px-4 py-5 font-sans">
                ...
                <motion.td className="px-8 py-5 text-right">
                  <motion.div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    <motion.button 
                      onClick={() => handleRun(rule)}
                      className="p-2 rounded-lg hover:bg-[var(--interactive-active)] text-[var(--text-muted)] hover:text-[var(--brand-primary)] transition-colors"
              ...
              <motion.tr>
                <motion.td colSpan={5} className="px-8 py-20 text-center text-[var(--text-muted)] font-sans">
                  <motion.div className="flex flex-col items-center gap-4 opacity-40">
                    <Zap size={48} />
                    <motion.p className="text-sm font-medium">No reactive pipelines defined yet.</motion.p>
                  </motion.div>
                </motion.td>
              </motion.tr>
            )}
